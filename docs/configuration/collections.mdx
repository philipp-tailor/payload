...
| `useAsUrlIdentifier`         | Specify a unique field (e.g., `slug`) to use in Admin Panel URLs instead of the document ID. The field must be `unique`, of type `text` or `number`, and not virtual or a relationship. Defaults to `id`. [More details](#use-as-url-identifier). |
...
### Use As URL Identifier

By default, Payload uses the document's `id` field in Admin Panel URLs (e.g., `/admin/collections/pages/123`). The `useAsUrlIdentifier` option allows you to use a different field, such as a `slug`, to create more user-friendly URLs (e.g., `/admin/collections/pages/hello-world`).
...
The field specified in `useAsUrlIdentifier` must meet the following criteria:

- **Unique**: Must have `unique: true` to ensure no URL conflicts
- **Type**: Must be `text` or `number` type
- **Not virtual**: Cannot be a virtual field
- **Not a relationship**: Cannot be a relationship or upload field
- **Top-level**: Cannot be a nested field path

**Behavior:**

- **Admin Panel Only**: This setting only affects Admin Panel URLs. REST API endpoints always use the document's `id` field.
- **Automatic URL Updates**: If you change the identifier field value (e.g., update the slug), the Admin Panel URL will automatically update after save.
- **Special Characters**: URLs are automatically encoded, so identifiers with spaces or special characters work correctly.
- **Performance**: Consider adding `index: true` to the identifier field for better query performance.

**Example URL Structure:**

```
# With default 'id'
/admin/collections/pages/123

# With useAsUrlIdentifier: 'slug'
/admin/collections/pages/hello-world
```

<Banner type="success">
  **Tip:** This feature is ideal for Collections where editors work with
  human-readable identifiers like slugs, SKUs, or usernames. It makes the Admin
  Panel more intuitive while keeping the REST API unchanged.
</Banner>
